...
Instructions: ${o}`):i.videoBase64?(u=[{type:"text",text:`${o}
Follow the instructions to generate a caption for this video.`},{type:"video_url",video_url:{url:i.videoBase64}}],h=`You are a captioning model. You will be given a video and you will need to generate a caption for it.
...
Instructions: ${o}`):(u=[{type:"text",text:`${o}
${i.type==="video"?`These are frames uniformly extracted from a video. Return a single caption to describe the video.`:""}
Follow the instructions to generate a caption.`}],i.dataUrls&&Array.isArray(i.dataUrls)&&u.push(...i.dataUrls.map(e=>({type:"image_url",image_url:{url:e,detail:"auto"}}))),h=`You are a captioning model. You will be given a picture or sequence of video frames and you will need to generate a caption for them.
...
Instructions: ${o}`);const m={model:s,messages:[{role:"system",content:h},{role:"user",content:u}],temperature:Number.isFinite(r)?r:.7,top_p:Number.isFinite(c)?c:1};n.useCustomEndpoint||(m.stream=!0,m.provider={ignore:["alibaba"],sort:"throughput"},R(s)&&e.reasoningToggle&&e.reasoningToggle.checked&&(m.reasoning={enabled:!0}));const f={"Content-Type":"application/json","HTTP-Referer":"https://obsxrver.com/DatasetCaption","X-Title":"DatasetCaption"};if(n.useCustomEndpoint)if(n.customUsername&&n.customPassword){const e=btoa(`${n.customUsername}:${n.customPassword}`);f.Authorization=`Basic ${e}`}else n.customBearerToken&&(f.Authorization=`Bearer ${n.customBearerToken}`);else f.Authorization=`Bearer ${t}`;const d=await fetch(ie(),{method:"POST",headers:f,body:JSON.stringify(m),signal:a});if(!d.ok){const e=await d.text().catch(()=>"");throw new Error(`HTTP ${d.status}: ${e||d.statusText}`)}let l;if(n.useCustomEndpoint){let e;try{e=await d.text(),e=e.trim()}catch(e){throw new Error(`Failed to read response: ${e.message}`)}let t;try{t=JSON.parse(e)}catch(t){throw new Error(`Failed to parse JSON response: ${t.message}. Response preview: ${e.substring(0,200)}...`)}l=t?.choices?.[0]?.message?.content}else{if(!d.body)throw new Error("Streaming response body is unavailable");const s=d.body.getReader(),o=new TextDecoder;let t="",e="",n=!1;const i=t=>{if(typeof t=="string"){e+=t;return}if(Array.isArray(t))for(const n of t)n&&n.type==="text"&&typeof n.text=="string"&&(e+=n.text)};for(;!0;){const{value:r,done:e}=await s.read();t+=o.decode(r||new Uint8Array,{stream:!e});const a=t.split(/\r?\n/);t=a.pop()||"";for(const s of a){if(!s.startsWith("data:"))continue;const t=s.slice(5).trim();if(!t)continue;if(t==="[DONE]"){n=!0;continue}let e;try{e=JSON.parse(t)}catch{continue}if(e?.error){const t=e.error?.message||JSON.stringify(e.error);throw new Error(`OpenRouter stream error: ${t}`)}const o=e?.choices?.[0]?.delta?.content;i(o)}if(e)break}if(!n&&e.trim().length===0)throw new Error("No caption returned");l=e}if(!l)throw new Error("No caption returned");if(typeof l=="string"&&l.includes("</think>")){const e=l.lastIndexOf("</think>");e!==-1&&(l=l.substring(e+"</think>".length).trim(),l=l.replace(/^\s+/,""))}if(Array.isArray(l)){const t=l.find(e=>e.type==="text"),e=t?.text||JSON.stringify(l);if(e.includes("</think>")){const t=e.lastIndexOf("</think>");if(t!==-1)return e.substring(t+"</think>".length).trim().replace(/^\s+/,"")}return e}return l}async function ce(){try{const e=++t.openRouterFetchSeq,n=await fetch("https://openrouter.ai/api/v1/models"),s=await n.json();if(e!==t.openRouterFetchSeq)return;t.openRouterModels=s.data.filter(e=>e.architecture&&e.architecture.input_modalities&&e.architecture.input_modalities.includes("image")).map((e,t)=>({...e,originalIndex:t,isLocal:!1})),K(),p()}catch(t){console.error("Failed to fetch models:",t),e.progressText.textContent="Error: Could not load models"}}function L(e,t=null){return e?t&&t.isLocal?"local":e.split("/")[0]:"unknown"}function R(e){const n=t.models.find(t=>t.id===e);return!!n&&!!n.supported_parameters&&n.supported_parameters.includes("reasoning")}function O(e){const n=t.models.find(t=>t.id===e);return!(!n||!n.architecture||!n.architecture.input_modalities)&&n.architecture.input_modalities.includes("video")}function c(){if(!e.modelOptions)return;const s=e.providerFilter?.value||"all",o=(e.modelSearch?.value||"").toLowerCase(),i=e.sortOrder?.value||"chronological",r=e.videoFilter?.checked;let n=t.models.filter(e=>{const t=L(e.id,e),n=s==="all"||t===s,i=e.id.toLowerCase().includes(o)||e.name&&e.name.toLowerCase().includes(o),a=!r||O(e.id);return n&&i&&a});if(i==="alphabetical"?n.sort((e,t)=>e.id.localeCompare(t.id)):i==="chronological"&&n.sort((e,t)=>e.originalIndex-t.originalIndex),e.modelOptions.innerHTML="",n.length===0){e.modelOptions.innerHTML='<div class="custom-option">No models found</div>';return}n.forEach(t=>{const n=document.createElement("div");n.className="custom-option",n.dataset.value=t.id,t.id===e.modelId.value&&n.classList.add("selected");const s=O(t.id),o=s?'<span title="Supports video input" style="cursor:help; margin-left:4px;">📹</span>':"";n.innerHTML=`
          <div class="model-name">${t.name||t.id}${o}</div>
          <div class="model-provider">${L(t.id,t)}</div>
        `,n.addEventListener("click",()=>{a(t.id),e.customSelect.classList.remove("open")}),e.modelOptions.appendChild(n)})}function a(n){const s=t.models.find(e=>e.id===n);if(!s)return;e.modelId.value=s.id,e.selectedModelName&&(e.selectedModelName.textContent=s.name||s.id),document.querySelectorAll(".custom-option").forEach(e=>{e.classList.remove("selected")});const i=document.querySelector(`.custom-option[data-value="${CSS.escape(s.id)}"]`);i&&(i.classList.add("selected"),localStorage.setItem(o.selectedModel,s.id),s.isLocal?localStorage.setItem(o.selectedModelLocal,s.id):localStorage.setItem(o.selectedModelOpenRouter,s.id));let a=n.split("/")[0];e.reasoningToggleField&&(R(n)&&(a==="google"||a==="anthropic")?e.reasoningToggleField.style.display="":e.reasoningToggleField.style.display="none")}function fe(){if(!e.customSelect||!e.customSelectTrigger)return;e.customSelectTrigger.addEventListener("click",()=>{e.customSelect.classList.toggle("open")}),document.addEventListener("click",t=>{e.customSelect.contains(t.target)||e.customSelect.classList.remove("open")}),e.providerFilter&&e.providerFilter.addEventListener("change",c),e.modelSearch&&e.modelSearch.addEventListener("input",c),e.sortOrder&&e.sortOrder.addEventListener("change",c),e.videoFilter&&e.videoFilter.addEventListener("change",c),e.modeToggle&&e.modeToggle.addEventListener("change",V)}me(),fe(),ce(),T()})()